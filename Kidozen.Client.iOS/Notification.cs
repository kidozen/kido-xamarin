...
namespace KidoZen
{
    public partial class Notification
    {
        ...
        {
			public SubscriptionBody(string subscriptionId)
            {
                platform = "apns";
				this.subscriptionId = subscriptionId;
				this.deviceId = GetDeviceMacAddress();
            }
		...
		{
			var pa =	NetworkInterface.GetAllNetworkInterfaces ().ToList ().FirstOrDefault (
				(ni) => (ni.NetworkInterfaceType == NetworkInterfaceType.Wireless80211 || ni.NetworkInterfaceType == NetworkInterfaceType.Ethernet));

			return BitConverter.ToString(pa.GetPhysicalAddress().GetAddressBytes());
		}
        ...
        public async Task<ServiceEvent<JToken>> Subscribe(string channelName, string deviceToken)
        {
            if (string.IsNullOrWhiteSpace(channelName)) throw new ArgumentNullException("channelName");
			return await doSubscribe(channelName, deviceToken);
        }
